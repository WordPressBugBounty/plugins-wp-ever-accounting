...
class Cancel_Command extends \ActionScheduler_WPCLI_Command {
	...
	protected function cancel_single( $hook, $callback_args, $group ) {
		if ( empty( $hook ) ) {
			\WP_CLI::error( __( 'Please specify hook of action to cancel.', 'wp-ever-accounting' ) );
		}
		...
		if ( null === $result ) {
			$e = new \Exception( __( 'Unable to cancel scheduled action: check the logs.', 'wp-ever-accounting' ) );
			$this->print_error( $e, false );
		}
	...
	protected function cancel_all( $hook, $callback_args, $group ) {
		if ( empty( $hook ) && empty( $group ) ) {
			\WP_CLI::error( __( 'Please specify hook and/or group of actions to cancel.', 'wp-ever-accounting' ) );
		}
		 ...
		 */
		\WP_CLI::success( __( 'Request to cancel scheduled actions completed.', 'wp-ever-accounting' ) );
	}
	...
	protected function print_success() {
		\WP_CLI::success( __( 'Scheduled action cancelled.', 'wp-ever-accounting' ) );
	}
	...
	protected function print_error( \Exception $e, $multiple ) {
		\WP_CLI::error(
			sprintf(
				...
				__( 'There was an error cancelling the %1$s: %2$s', 'wp-ever-accounting' ),
				$multiple ? __( 'scheduled actions', 'wp-ever-accounting' ) : __( 'scheduled action', 'wp-ever-accounting' ),
				$e->getMessage()
			)
		);
	}
