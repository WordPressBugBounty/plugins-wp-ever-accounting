	...
	public function execute() {
		...
		$progress_bar = \WP_CLI\Utils\make_progress_bar(
			sprintf(
				...
				_n( 'Deleting %d action', 'Deleting %d actions', $this->action_counts['total'], 'wp-ever-accounting' ),
		...
		$format = _n( 'Deleted %1$d action', 'Deleted %1$d actions', $this->action_counts['deleted'], 'wp-ever-accounting' ) . ', ';
		/* translators: %2$d: number of actions deletions failed */
		$format .= _n( '%2$d failure.', '%2$d failures.', $this->action_counts['failed'], 'wp-ever-accounting' );
