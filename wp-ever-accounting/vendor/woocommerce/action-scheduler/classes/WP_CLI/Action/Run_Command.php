	...
	public function execute() {
		...
		$progress_bar = \WP_CLI\Utils\make_progress_bar(
			sprintf(
				...
				_n( 'Executing %d action', 'Executing %d actions', $this->action_counts['total'], 'wp-ever-accounting' ),
		...
		foreach ( array(
			 ...
			 */
			$format = _n( '%1$d action %2$s.', '%1$d actions %2$s.', $count, 'wp-ever-accounting' );
		...
		\WP_CLI::success(
			sprintf(
				...
				_n( 'Executed %d action.', 'Executed %d actions.', $this->action_counts['executed'], 'wp-ever-accounting' ),
