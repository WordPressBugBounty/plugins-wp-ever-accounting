...
class ActionScheduler_ActionFactory {
	...
	public function repeat( $action ) {
		...
		if ( is_null( $next ) || ! $schedule->is_recurring() ) {
			throw new InvalidArgumentException( __( 'Invalid action - must be a recurring action.', 'wp-ever-accounting' ) );
		}
		...
		try {
			...
			error_log(
				sprintf(
					...
					__( 'Caught exception while enqueuing action "%1$s": %2$s', 'wp-ever-accounting' ),
					$options['hook'],
					$e->getMessage()
				)
			);
