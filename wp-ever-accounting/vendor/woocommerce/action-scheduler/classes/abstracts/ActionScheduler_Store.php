...
abstract class ActionScheduler_Store extends ActionScheduler_Store_Deprecated {
	...
	protected function validate_action( ActionScheduler_Action $action ) {
		if ( strlen( wp_json_encode( $action->get_args() ) ) > static::$max_args_length ) {
			...
			throw new InvalidArgumentException( sprintf( __( 'ActionScheduler_Action::$args too long. To ensure the args column can be indexed, action args should not be more than %d characters when encoded as JSON.', 'wp-ever-accounting' ), static::$max_args_length ) );
		}
	}
	...
	public function get_status_labels() {
		return array(
			self::STATUS_COMPLETE => __( 'Complete', 'wp-ever-accounting' ),
			self::STATUS_PENDING  => __( 'Pending', 'wp-ever-accounting' ),
			self::STATUS_RUNNING  => __( 'In-progress', 'wp-ever-accounting' ),
			self::STATUS_FAILED   => __( 'Failed', 'wp-ever-accounting' ),
			self::STATUS_CANCELED => __( 'Canceled', 'wp-ever-accounting' ),
		);
	}
