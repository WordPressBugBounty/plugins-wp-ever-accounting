...
class ActionScheduler_wpPostStore extends ActionScheduler_Store {
	...
	protected function save_post_array( $post_array ) {
		...
		if ( is_wp_error( $post_id ) || empty( $post_id ) ) {
			throw new RuntimeException( __( 'Unable to save action.', 'wp-ever-accounting' ) );
		}
		return $post_id;
	}
	...
	protected function get_query_actions_sql( array $query, $select_or_count = 'select' ) {
		...
		if ( ! in_array( $select_or_count, array( 'select', 'count' ), true ) ) {
			throw new InvalidArgumentException( __( 'Invalid schedule. Cannot save action.', 'wp-ever-accounting' ) );
		}
	...
	protected function claim_actions( $claim_id, $limit, ?DateTime $before_date = null, $hooks = array(), $group = '' ) {
		...
		if ( false === $rows_affected ) {
			throw new RuntimeException( __( 'Unable to claim actions. Database error.', 'wp-ever-accounting' ) );
		}
	...
	protected function get_actions_by_group( $group, $limit, DateTime $date ) {
		...
		if ( ! term_exists( $group, self::GROUP_TAXONOMY ) ) {
			...
			throw new InvalidArgumentException( sprintf( __( 'The group "%s" does not exist.', 'wp-ever-accounting' ), $group ) );
		}
	...
	public function release_claim( ActionScheduler_ActionClaim $claim ) {
		...
		if ( false === $result ) {
			...
			throw new RuntimeException( sprintf( __( 'Unable to unlock claim %s. Database error.', 'wp-ever-accounting' ), $claim->get_id() ) );
		}
	}
	...
	public function unclaim_action( $action_id ) {
		...
		if ( false === $result ) {
			...
			throw new RuntimeException( sprintf( __( 'Unable to unlock claim on action %s. Database error.', 'wp-ever-accounting' ), $action_id ) );
		}
	}
	...
	public function mark_failure( $action_id ) {
		...
		if ( false === $result ) {
			/* translators: %s: action ID */
			throw new RuntimeException( sprintf( __( 'Unable to mark failure on action %s. Database error.', 'wp-ever-accounting' ), $action_id ) );
		}
	}
	...
	protected function validate_action( ActionScheduler_Action $action ) {
		try {
			...
			$message = sprintf( __( '%s Support for strings longer than this will be removed in a future version.', 'wp-ever-accounting' ), $e->getMessage() );
