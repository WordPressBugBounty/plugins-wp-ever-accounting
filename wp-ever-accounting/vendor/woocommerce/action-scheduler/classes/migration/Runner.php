...
class Runner {
	...
	public function run( $batch_size = 10 ) {
		...
		if ( $this->progress_bar ) {
			...
			$this->progress_bar->set_message( sprintf( _n( 'Migrating %d action', 'Migrating %d actions', $batch_size, 'wp-ever-accounting' ), $batch_size ) );
			...
			if ( $destination_action_id ) {
				$this->destination_logger->log(
					...
					sprintf(
						...
						__( 'Migrated action with ID %1$d in %2$s to ID %3$d in %4$s', 'wp-ever-accounting' ),
						$source_action_id,
