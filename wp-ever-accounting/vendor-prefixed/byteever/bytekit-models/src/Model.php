...
use EverAccounting\ByteKit\Models\Traits\HasAttributes;
use EverAccounting\ByteKit\Models\Traits\HasMetaData;
use EverAccounting\ByteKit\Models\Traits\HasRelations;
/**
...
abstract class Model
{
    ...
    protected function booting()
    {
        ...
        if (empty($this->columns)) {
            $this->columns = $wpdb->get_col("DESCRIBE {$wpdb->prefix}{$this->table}");
    ...
    protected function read($id)
    {
        ...
        if (false === $data) {
            ...
            foreach ($args as $key => $value) {
                if (is_array($value)) {
                    $where[] = $wpdb->prepare("`{$key}` IN (" . implode(',', array_fill(0, count($value), '%s')) . ')', $value);
                    // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Prepared.
